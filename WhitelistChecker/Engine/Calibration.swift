...
struct CalibrationSnapshot {
    ...
    /// Сеть душит чужой трафик: даже лучший чужой эталон медленнее белого более
    /// чем в 5 раз. Разница меньше — это погрешность измерения (VPN, джиттер
    /// канала, деление полосы между эталонами), а не шейп.
    var shaping: Bool {
        whiteBps > 0 && foreignBps > 0 && foreignBps < whiteBps / 5
    }
